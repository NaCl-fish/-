...
def cnn_inference(images, batch_size, n_classes, is_training):
    ...
    with tf.variable_scope('conv1') as scope:
        ...
        gn1 = groupnormalization.GroupNormalization(groups = GROUP)(pre_activation)

        conv1 = tf.nn.relu(gn1, name='conv1')   # 用relu激活函数非线性化处理
    ...
    with tf.variable_scope('conv2') as scope:
        weights = tf.get_variable('weights',
                                  shape=[3, 3, 16, 16],  # 这里的第三位数字GROUP需要等于上一层的tensor维度
                                  dtype=tf.float32,
                                  initializer=tf.truncated_normal_initializer(stddev=0.1, dtype=tf.float32))
        biases = tf.get_variable('biases',
                                 shape=[16],
        ...
        gn2 = groupnormalization.GroupNormalization(groups = GROUP)(pre_activation)

        conv2 = tf.nn.relu(gn2, name='conv2')
    ...
    with tf.variable_scope('local3') as scope:
        ...
        local3 = tf.nn.relu(groupnormalization.GroupNormalization(groups = GROUP)(tf.matmul(reshape, weights) + biases),
                            name='local3')
    ...
    with tf.variable_scope('local4') as scope:
        ...
        biases = tf.get_variable('biases',
                                 ...
                                 initializer=tf.constant_initializer(0.1))
        # 矩阵相乘再加上biases，用relu激活函数非线性化处理
        local4 = tf.nn.relu(groupnormalization.GroupNormalization(groups = GROUP)(tf.matmul(local3, weights) + biases),
                            name='local4')
