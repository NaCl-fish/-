...
def cnn_inference(images, batch_size, n_classes, is_training):
    ...
    with tf.variable_scope('conv1') as scope:
        ...
        ln1 = tf2.keras.layers.LayerNormalization(trainable=is_training)(pre_activation)
    ...
    with tf.variable_scope('conv2') as scope:
        ...
        ln2 = tf2.keras.layers.LayerNormalization(trainable=is_training)(pre_activation)
    ...
    with tf.variable_scope('local3') as scope:
        ...
        local3 = tf.nn.relu(tf2.keras.layers.LayerNormalization(trainable=is_training)(tf.matmul(reshape, weights) + biases),
                            name='local3')
    ...
    with tf.variable_scope('local4') as scope:
        ...
        biases = tf.get_variable('biases',
                                 ...
                                 initializer=tf.constant_initializer(0.1))
        # 矩阵相乘再加上biases，用relu激活函数非线性化处理
        local4 = tf.nn.relu(tf2.keras.layers.LayerNormalization(trainable=is_training)(tf.matmul(local3, weights) + biases),
                            name='local4')
