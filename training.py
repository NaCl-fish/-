...
IMG_H = 128
BATCH_SIZE = 16  # 每批次读取数据的数量（调参）
CAPACITY = 2000  # 队列最大容量
MAX_STEP = 20000  # 训练最大步数（调参）
learning_rate = 0.0001  # 学习率，一般小于0.0001（调参）
...
train_logits = model.cnn_inference(train_batch, BATCH_SIZE, N_CLASSES, True)
...
with tf.Session() as sess:
    ...
    saver = tf.train.Saver(var_list=tf.global_variables(), max_to_keep=25) # 用于存储训练好的模型
    ...
    try:
        ...
        for step in np.arange(MAX_STEP):
            ...
            # 每隔1000步，保存一次训练好的模型（即：训练好的模型的参数保存下来）
        ...
        plt.legend()                    # 给图加上图例
        plt.savefig('./image/Accuracy&Loss')
        plt.show()
